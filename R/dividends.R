...
get_dividends <- function(ticker, exchange, token, country) {
  cli::cli_alert_info("fetching dividends for ticker {ticker}|{exchange}|{country}")

  f_out <- fs::path(
    dir_dataout,
    country,
    glue::glue("{ticker}_eod-dividends.csv")
  )
